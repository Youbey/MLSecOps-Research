...
# Ensure utils is in path if running directly
...
class FLServer:
    def __init__(self):
        ...
        # Initialize Metrics
        metrics.update_round(self.round)
        metrics.update_threat_level(0)
    ...
    def register_client(self, client_id, num_samples, token=None, public_key_pem=None):
        ...
        if client_id not in self.client_states:
            ...
            # UPDATE METRICS
            metrics.update_client_count(len(self.client_states))
    ...
    def process_update(self, client_id, weights, metrics):
        ...
        metrics.record_update_received(client_id)
        if metrics_data:
            metrics.update_client_metrics(client_id, metrics_data.get('loss'), metrics_data.get('accuracy'))
        ...
        # UPDATE METRICS: Accepted
        metrics.record_update_accepted(client_id)
